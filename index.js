...
function openFullScreen(videoId) {
    ...
    // Remove full-screen when clicked outside of the video
    overlay.addEventListener('click', function() {
        document.body.removeChild(overlay);
    });
    
}
// Open the Create Post popup
function openPopup() {
...
function closePopupOutside(event) {
    if (event.target === document.getElementById('createPostPopup')) {
        closePopup();
    }
}
...
function closePopup() {
    ...
    document.getElementById('filePreview').innerHTML = ''; // Clear the file preview
...
function handlePostSubmit(event) {
    ...
    .then(data => {
        ...
        displayNewPost(data.post); // Display the new post on the page
        closePopup(); // Close the popup after successful submission
        document.getElementById('successMessage').style.display = 'block'; // Show success message
    })
    .catch(error => console.error('Error submitting post:', error)); // Log errors
}

// Function to display the new post dynamically on the page
function displayNewPost(post) {
            ...
            <div class="photo">
                ${post.files.map(file => {
                    if (file.type.startsWith('image/')) {
                        return `<img src="${file.url}" alt="Post image">`;
    ...
    postContainer.innerHTML = postHTML + postContainer.innerHTML; // Add new post to the top
}

// Handle file previews before posting
function previewFiles() {
    ...
    Array.from(files).forEach((file) => {
        ...
        reader.onloadend = function() {
            let mediaElement;
            if (file.type.startsWith('image/')) {
                mediaElement = `<img src="${reader.result}" alt="Image preview">`;
            } else if (file.type.startsWith('video/')) {
                mediaElement = `<video controls src="${reader.result}"></video>`;
            }
            preview.innerHTML += `<div>${mediaElement}</div>`;
        };
                ...
                const postHTML = `
                    <div class="feed">
                        ...
                        <div class="action-buttons">
                            <i id="like-icon" calss="fas fa-heart"></i>
                    </div>
                `;
